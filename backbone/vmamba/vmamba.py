...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        assert selective_scan_backend in [None, "core", "oflex", "mamba", "torch"]
...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i, layer in enumerate(self.layers):
            # 1. 逐个执行 Block，但如果遇到了 downsample 层则跳过
            for block in layer.blocks:
                if layer.downsample is not None and block is layer.downsample:
                    continue  # 跳过
                x = block(x)

            # 2. 此时 x 是当前 Stage 完整的特征 (未下采样)
...
"""
參數詳解：
    depths：网络深度。表示 4 个 Stage 中，每个 Stage 包含多少个 VSSBlock。
    dims：特征维度。Stage 1 的输出通道数。
    drop_path_rate：随机深度失活率 (Stochastic Depth)。随着网络加深，随机丢弃整个 Residual Block 的概率。这是一种正则化手段。
    patch_size：Patch 大小。第一层卷积的 Kernel Size 和 Stride。这是 ViT/Swin 的标准设定，不要动。
    in_chans：输入通道数。第一层 Patch Embedding 的输入维度。
    num_classes：分类头类别数。必须填 1000。为了匹配 ImageNet 预训练权重里的那个 Head，否则加载权重时会报 Size Mismatch。
    ssm_d_state：SSM 状态维度。Mamba 内部的状态空间大小 (N)。类似于 RNN 的隐藏层大小，决定记忆容量。
    ssm_ratio：扩张比率。SSM 内部特征维度相对于输入维度的倍数。d_inner = d_model * ssm_ratio。2.0 是标准，1.0 是轻量版。
    ssm_dt_rank：dt 投影秩。Delta 参数的低秩投影维度。
    ssm_act_layer：SSM 激活函数。在卷积和门控分支中使用的激活函数。不要改成 ReLU/GELU，因为权重是基于 SiLU 训练的。
    ssm_conv：局部卷积核大小。SS2D 内部 Depthwise Conv 的 kernel size。用来提取局部特征，弥补 Mamba 只关注全局的不足。
    ssm_conv_bias：积偏置。上述局部卷积是否带 Bias (b)。
    ssm_drop_rate：SSM 内部 Dropout。在 SS2D 投影层后的 Dropout 率。Mamba 结构比较紧凑，很少在内部加 Dropout。
    ssm_init：初始化策略。決定 A, D, dt 等 SSM 參數的初始化分佈。v0: 复杂的 S4D 初始化 (推荐); v1/v2: 简单的随机初始化。
    forward_type：前向传播版本。决定使用哪种 SS2D 实现逻辑。由 版本号 (Prefix) 和 功能后缀 (Suffix) 组成。
        版本号 (Prefix)：
            v0：使用 SS2Dv0 类 (老旧) 原始版。逻辑简单，显存优化差，不支持很多新特性。Vanilla 默认配置。
            v2：SS2Dv2 + backend="core" 标准版。使用 PyTorch 核心算子，兼容性好。
            v5：SS2Dv2 + no_einsum=True 极速版。把 einsum 操作换成了 Linear 或 Conv1d。速度更快，显存更省。
        功能后缀 (Suffix)：
            _noz：No Z (No Gating) 去除门控分支。输入投影维度减半，参数量减少，计算变快，但非线性能力减弱。
            _no32：Disable Force FP32 允许混合精度。允许在 FP16/BF16 下运行，进一步提速。
            _ondwconv3：Output Norm: DWConv 改变输出层的归一化方式（通常不用，默认是 LayerNorm）。
    mlp_ratio：MLP 扩张比。Block 中是否包含 Feed-Forward Network (FFN)。0.0 表示不使用 MLP (纯 Mamba 架构)；4.0 表示混合架构。
    mlp_act_layer：MLP 激活函数。如果开启 MLP，这里用什么激活。若 mlp_ratio=0.0，則这个参数無效。
    mlp_drop_rate：MLP Dropout。MLP 内部的 Dropout。mlp_ratio=0.0 同上。
    gmlp：Gated MLP 开关。是否使用 gMLP (带有门控机制的 MLP)。VMamba 通常只用 Mamba 模块，或者标准的 MLP，不用 gMLP 这种变体。
    patch_norm：Patch 归一化。在第一层 PatchEmbed 输出后是否加 LayerNorm。必须 True。
    norm_layer：归一化层类型。Block 结尾用的归一化层。别动。
    downsample_version：下采样模块版本。Stage 之间的特征图缩小方式。v1: PatchMerging (类似 Swin); v3: 3x3 Conv (更好适配分割)。
    patchembed_version：PatchEmbed 实现版本。v1 是标准 Conv2d；v2 是更复杂的堆叠结构。
    use_checkpoint：梯度检查点 (Activation Checkpointing)。是否用“时间换空间”来省显存。如果你显存够，就保持 False 跑得快。
    posembed：绝对位置编码。是否加可学习的 Positional Embedding。必须 False。VMamba 依靠 SSM 的因果性和卷积的局部性来感知位置，不需要显式的 PosEmbed。
    imgsize：输入分辨率。
"""
...
def vmamba_tiny_s1l8(channel_first=True):
    return VSSM(
        ...
        patch_norm=True, norm_layer=("ln2d" if channel_first else "ln"),
        downsample_version="v3", patchembed_version="v2",
        use_checkpoint=False, posembed=False, imgsize=224, out_indices = (0, 1, 2, 3),
    )
